...
export const narration = 'text-top-down-letters';
...
export default [
  {
    id: 'text-top-down-letters',
    chapter: 'text-top-down-letters',
    duration: 0.2,
    effect: async () => {
      const gsap = await loadGsap({ flip: false, motionPath: false });
      const grain = mountGrain({ opacity: 0.03, seed: 1, zIndex: 60 });
      const reveal = mountTextReveal('WPForms 1.9.9+', { preset: 'top-down-letters' });
